...
public class GameSessionController implements MoleButtonInterface {
    ...
    public void didHitMoleAtIndex(int index) {
        if (board.getMoleAtIndex(index).getState() == MoleState.VISIBLE
                || board.getMoleAtIndex(index).getState() == MoleState.ALMOST_VISIBLE
                || board.getMoleAtIndex(index).getState() == MoleState.SLIGHTLY_VISIBLE) {
